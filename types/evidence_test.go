...
func TestMaxEvidenceBytes(t *testing.T) {
	forAllPrivKeyTypes(t, func(t *testing.T, name string, kt PrivKeyType) {
		val := NewMockPV(kt)
		randomPartHash := rand.Bytes(int(rand.Uint32() % 1000))
		randomHash1 := rand.Bytes(int(rand.Uint32() % 1000))
		randomHash2 := rand.Bytes(int(rand.Uint32() % 1000))
		blockID := makeBlockID(tmhash.Sum(randomHash1), math.MaxInt64, tmhash.Sum(randomPartHash))
		blockID2 := makeBlockID(tmhash.Sum(randomHash2), math.MaxInt64, tmhash.Sum(randomPartHash))
		const chainID = "mychain"
