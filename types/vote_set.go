...
func NewVoteSet(chainID string, height int64, round int32,
	signedMsgType tmproto.SignedMsgType, voterSet *VoterSet) *VoteSet {
	if height == 0 {
...
func (voteSet *VoteSet) addVote(vote *Vote, execVoteVerify func(chainID string,
	...
	if voter == nil {
		return false, fmt.Errorf(
			"cannot find voter %d in voterSet of size %d: %w",
			valIndex, voteSet.voterSet.Size(), ErrVoteInvalidValidatorIndex)
	}
	...
	added, conflicting := voteSet.addVerifiedVote(vote, blockKey, voter.VotingWeight)
	if conflicting != nil {
...
func (voteSet *VoteSet) addVerifiedVote(
	...
	quorum := voteSet.voterSet.TotalVotingWeight()*2/3 + 1
...
func (voteSet *VoteSet) HasTwoThirdsAny() bool {
	...
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
	return voteSet.sum > voteSet.voterSet.TotalVotingWeight()*2/3
}
...
func (voteSet *VoteSet) HasAll() bool {
	...
	voteSet.mtx.Lock()
	defer voteSet.mtx.Unlock()
	return voteSet.sum == voteSet.voterSet.TotalVotingWeight()
}
...
func (voteSet *VoteSet) sumTotalFrac() (int64, int64, float64) {
	voted, total := voteSet.sum, voteSet.voterSet.TotalVotingWeight()
