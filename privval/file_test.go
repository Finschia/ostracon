...
var testingKeyTypes = []struct {
	...
	{
		keyType: PrivKeyTypeComposite,
		privKey: *composite.GenPrivKey(),
		pubKey:  composite.PubKey{},
	},
}
...
func TestGenFilePV(t *testing.T) {
	...
	for _, testing := range testingKeyTypes {
		privVal, err := GenFilePV(tempKeyFile.Name(), tempStateFile.Name(), testing.keyType)
		require.Nil(t, err)
		require.EqualValues(t, reflect.TypeOf(testing.pubKey), reflect.TypeOf(privVal.Key.PubKey))
	}
...
func TestLoadOrGenValidator(t *testing.T) {
	...
	for _, testing := range testingKeyTypes {
		...
		privVal, err := LoadOrGenFilePV(tempKeyFilePath, tempStateFilePath, testing.keyType)
		require.Nil(t, err)
		...
		assert.Equal(addr, privVal.GetAddress(), "expected privval addr to be the same", "keyType", testing.keyType)
	}
}
