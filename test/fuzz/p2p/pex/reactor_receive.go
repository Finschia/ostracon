...
func init() {
	...
	pexR := pex.NewReactor(addrB, cfg.RecvAsync, &pex.ReactorConfig{SeedMode: false, RecvBufSize: cfg.PexRecvBufSize})
	if pexR == nil {
...
func Fuzz(data []byte) int {
	...
	cfg := config.DefaultP2PConfig()
	cfg.PexReactor = true
	sw := p2p.MakeSwitch(cfg, 0, "127.0.0.1", "123.123.123", func(i int, sw *p2p.Switch, cfg *config.P2PConfig) *p2p.Switch {
		return sw
	})
	pexR.SetSwitch(sw)
