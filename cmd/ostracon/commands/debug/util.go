...
// dumpNetInfo gets network information state dump from the Ostracon RPC and
...
// dumpConsensusState gets consensus state dump from the Ostracon RPC and
// writes it to file. It returns an error upon failure.
...
// copyWAL copies the Ostracon node's WAL file. It returns an error if the
...
// copyConfig copies the Ostracon node's config file. It returns an error if
