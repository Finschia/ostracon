...
func TestValidateBlockHeader(t *testing.T) {
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		proposerAddr := sm.SelectProposer(state.Validators, state.LastProofHash, height, 0).Address
		/*
...
func TestValidateBlockCommit(t *testing.T) {
	...
	for height := int64(1); height < validationTestsStopHeight; height++ {
		proposerAddr := sm.SelectProposer(state.Validators, []byte{}, height, 0).Address
		if height > 1 {
			/*
